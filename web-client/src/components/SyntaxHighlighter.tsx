...
function SyntaxHighlighter({
  ...
  const highlightedLines = content.map(line => language
    ? hljs.highlight(language, line)
          ...
          <Box>
            <Group>
              {
                ...
                <ActionIcon size="xs" onClick={() => setLinesWithPendingComment(prev => [...prev, i + 1])}><IconPlus /></ActionIcon>
              }

              <Text px="sm" bg="black" opacity={0.5}>{i + 1}</Text>
...

            {
              ...
              <Paper withBorder p="sm">
                <Textarea my="sm">This could be wrapped in a Boolean to make sure the type is a boolean instead of string. Isn't that in our latest style guideline, @ecekahraman?</Textarea>
